...
export class FSDatParser {
    ...
    consumeLevelMapDetails(offset) {
        ...
        let firstLayerBytes, objCode, tileCode, repititions, name, isEntity, funcName, layerMap, secondLayerBytes;
        ...
        while (nextOffset < endOfFirstLayerOffset) {
            ...
            if (objCode === 0xFF) {
                ...
                for (let i = currentTileMarker; i < currentTileMarker + repititions; i++) {
                    layerMap[funcName](
                        32 - i % 16, // x goes across from top right
                        Math.floor(i / 16), // y goes down from top right
                        name,
                        this.getRenderer()
                    );
                }
    ...
    getObjectCodeValue(code) {
        ...
        if (this.objectMap.has(code)) {
            ...
            if (ENTITY_NAME_LIST.indexOf(returnValue) !== -1) {
                return ["entity", returnValue];
            }
            return ["tile", returnValue];
        } else {
